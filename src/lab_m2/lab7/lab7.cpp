...
void Lab7::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix)
{
    ...
    glUniformMatrix4fv(shader->GetUniformLocation("bones_matrices"), 200, GL_FALSE, glm::value_ptr(bones[0]));
...
void Lab7::ComputeInterpolatedScaling(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    // Save the result in the out variable
    out = pNodeAnim->mScalingKeys[scalingIndex].mValue * (1 - factor) + factor * pNodeAnim->mScalingKeys[nextScalingIndex].mValue;
}
...
void Lab7::ComputeInterpolatedPosition(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    // Save the result in the out variable
    out = pNodeAnim->mPositionKeys[positionIndex].mValue * (1 - factor) + factor * pNodeAnim->mPositionKeys[nextPositionIndex].mValue;
}
